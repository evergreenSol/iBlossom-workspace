...
<body>
	...
	<div class="mypage-subscribe" id="subscribe-6m-show" style="text-align: center">
	    <span style="font-size: 16px; font-weight: 600;">
	        ${ sessionScope.loginUser.userName }님은
	        <span class="subscribe-name"> ${ list6m[0].subProductName } </span>
	        <span>을 구독중이십니다.</span>
	    </span>
	</div>
	
	<div id="subscribe-6m" class="subscribe-close">
                        
		<c:forEach var="i"  begin="0" end="${ list6m.size() -1 }" >
			<table>
	               <tr>
	                   <td colspan="2"><p style="font-size: 18px; font-weight: 600">${ list6m[i].subDate }</p></td>
	                   <td></td>
	                   <td width="490" align="right"></td>
	
	               </tr>
	               <tr>
	                   <td rowspan="5" width="100" height="100"><img src="${ list6m[i].subChangeName }" width="100px" height="100px"></td>
	                   <th width="80">상품명</th>
	                   <td width="10"></td>
	                   <td class="productName" width="80">
	                       ${ list6m[i].subProductName }
	                   </td>
	               </tr>
	               <tr>
	                   <th>받는분</th>
	                   <td></td>
	                   <td>${ list6m[i].subReceiverUser }</td>
	               </tr>
	               <tr>
	                   <th>구독내용</th>
	                   <td></td>
	                   <td class="subLevel">6개월 구독</td>
	               </tr>
	               <tr>
	                   <th>회차</th>
	                   <td></td>
	                   <!-- 만약에 구독개월스 컬럼이 0이면,  아래처럼 arraylist의 사이즈 뽑으면 될듯?-->
	                   <td>${i+1}회 / ${ list6m.size() }회</td>
		...
		<div style="margin-bottom: 18px;">
			...
			<div class="mypage-subscribe-cancel-area" style="display:inline; margin-left: 595px;">
				...
				<input type="hidden" value="${ list6m[0].receiptId }" name="receiptId">
			</div>
		...
		<c:forEach var="i"  begin="0" end="0" >
			<span style="font-size: 16px; font-weight: 600;">
				${ list6m[0].subReceiverUser }
				<span>${ list6m[0].subReceiverPhone }</span>
				<span>보내는 사람 (${ loginUser.userName })</span>
		    </span>
			<p style="font-size: 14px; margin: 0px;">[${ list6m[0].subReceiverUser }] ${ list6m[0].deliverTo }</p> 
		</c:forEach>                         
		</div>
	...
	<script>
		$(function () {
			...
			var subPrice = ${ list6m[0].subPrice }
